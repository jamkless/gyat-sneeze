...
local Drawing = loadstring(game:HttpGet("https://raw.githubusercontent.com/JeffreeyDahmer02/Storage/refs/heads/main/Drawing.lua"))();
...
local esppreview = prev.enabled

-- Theme Variables
--local themes = {}
local theme = {
    accent = Color3.fromRGB(227, 209, 228),
...
do
    ...
    function utility:Lerp(instance, instanceTo, instanceTime)
        ...
        --
        connection = utility:Connection(rs.PreSimulation, function(delta)
...
do
    ...
    function library:Loader(info)
        ...
        local size = info.size or info.Size or Vector2.new(200,360)
        ...
        theme.accent = accent
        --
        local window = {pages = {}, loader = true, isVisible = true, pageammount = pageammount, callback = callback, wminfo = "$$$$$ AntarcticaWare $$$$$ || UID : %u || Ping : %s || Fps : %u", currentPage = nil, fading = false, dragging = false, drag = Vector2.new(0,0), currentContent = {frame = nil, dropdown = nil, multibox = nil, colorpicker = nil, keybind = N, textbox = nil}}
        --
        local main_frame = Instance.new("ScreenGui")
main_frame.Name = utility:RandomString()
main_frame.Parent = gui_parent
main_frame.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
window["main_frame"] = frame
    ...
    self.main_frame.Position = UDim2.new(0, vector.X, 0, vector.Y)
    for i,v in pairs(self.visibleContent) do
        if v.Position then
            v.Position = utility:Position(v.Position.X.Scale, v.Position.X.Offset, v.Position.Y.Scale, v.Position.Y.Offset, self.main_frame)
        end
    end
end
        --
        ...
        function window:Cursor(info)
            ...
            --
            utility:Connection(rs.PreSimulation, function()
                local mouseLocation = utility:MouseLocation()
                --
        ...
        library.began[#library.began + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and window.isVisible and utility:MouseOverDrawing({main_frame.Position.X,main_frame.Position.Y,main_frame.Position.X + main_frame.Size.X,main_frame.Position.Y + 20}) then
                local mouseLocation = utility:MouseLocation()
                --
    ...
    if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and window.dragging then
...
library.changed[#library.changed + 1] = function(Input)
    if window.dragging and window.isVisible then
        ...
        local xClamp = math.clamp(mouseLocation.X - window.drag.X, 5, screenSize.X - main_frame.Size.X - 5)
        local yClamp = math.clamp(mouseLocation.Y - window.drag.Y, 5, screenSize.Y - main_frame.Size.Y - 5)
        window:Move(Vector2.new(xClamp, yClamp))
    end
end
...
utility:Connection(uis.InputChanged, function()
    ...
    function library:New(info)
		local info = info or {}
        local name = info.name or info.Name or info.title or info.Title or "UI Title"
        local size = Vector2.new(350,400)
        local accent = info.accent or info.Accent or info.color or info.Color or theme.accent
        local callback = info.callback or info.Callback or info.callBack or info.CallBack or function() end
        ...
        --
        window.UpdateTitle = function(name: string)
            title.Text = name
        end
        --
        library.colors[title] = {
        ...
        function window:Watermark(info)
            ...
            local Tick = tick()
            --
            utility:Connection(rs.PreSimulation, function(FPS)
    ...
    if window.dragging and window.isVisible then
        ...
        local move = Vector2.new(
            math.clamp(mouseLocation.X - window.drag.X, 5, utility:GetScreenSize().X - window.main_frame.Size.X - 5),
            math.clamp(mouseLocation.Y - window.drag.Y, 5, utility:GetScreenSize().Y - window.main_frame.Size.Y - 5)
        )
        window:Move(move)
    end
end
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    function library:Page(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and utility:MouseOverDrawing({page_button.Position.X,page_button.Position.Y,page_button.Position.X + page_button.Size.X,page_button.Position.Y + page_button.Size.Y}) and window.currentPage ~= page then
                page:Show()
            end
        end
        --
    ...
    function pages:MultiSection(info)
        ...
        for i,v in pairs(msections) do
            ...
            --
            library.began[#library.began + 1] = function(Input)
                if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and page.open and  utility:MouseOverDrawing({msection_frame.Position.X,msection_frame.Position.Y,msection_frame.Position.X + msection_frame.Size.X,msection_frame.Position.Y + msection_frame.Size.Y}) and multiSection.currentSection ~= msection and not window:IsOverContent() then
    ...
    function pages:PlayerList(info)
        ...
        for Index = 1, 1 do
            ...
            --
            library.began[#library.began + 1] = function(Input)
                if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and (button_outline.Visible or button.open) and window.isVisible then
                    if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and button_outline.Visible then
                        ...
                        elseif utility:MouseOverDrawing({button_outline.Position.X, button_outline.Position.Y, button_outline.Position.X + button_outline.Size.X, button_outline.Position.Y + button_outline.Size.Y}) and not window:IsOverContent() then
                            task.spawn(function()
                                utility:LoadImage(button_gradient, "gradientdown", "https://i.imgur.com/XYVD9Mh.png")
                                --
                                task.wait(0.15)
                                --
                                utility:LoadImage(button_gradient, "gradient", "https://i.imgur.com/XYVD9Mh.png")
                    ...
                    elseif (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and button.open then
                        button:Close()
                    end
                end
            end
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and list_outline.Visible and window.isVisible then
        ...
        --
        library.ended[#library.ended + 1] = function(Input)
            if playerList.scrolling[1] and (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
    ...
    function sections:Toggle(info)
        ...
        function toggle:Colorpicker(info)
            ...
            --
            library.began[#library.began + 1] = function(Input)
                if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and colorpicker_outline.Visible then
                ...
                elseif (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and colorpicker.open then
                    colorpicker.open = not colorpicker.open
                    --
                    for i,v in pairs(colorpicker.holder.drawings) do
                        utility:Remove(v)
        ...
        function toggle:Keybind(info)
            ...
            function keybind:Set(tbl)
                ...
                if tbl[3] then
                    ...
                    if keybind.mode == "Toggle" then
                        keybind.active = not keybind.active or false
                    else
            ...
            library.began[#library.began + 1] = function(Input)
                ...
                --
                if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and keybind_outline.Visible then
    ...
    function sections:Slider(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and slider_outline.Visible and window.isVisible and page.open and utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + slider.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + slider.axis + (name and 29 or 14)}) and not window:IsOverContent() then
                slider.holding = true
                slider:Refresh()
            end
        end
        --
        library.ended[#library.ended + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and slider.holding and window.isVisible then
    ...
    function sections:Button(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and button_outline.Visible and window.isVisible and utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + button.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + button.axis + 20}) and not window:IsOverContent() then
                task.spawn(function()
                    utility:LoadImage(button_gradient, "gradientdown", "https://i.imgur.com/XYVD9Mh.png")
                    --
                    task.wait(0.15)
...
function sections:Textbox(info)
    ...
    local placeholder = info.placeholder or info.Placeholder or info.placeHolder or info.PlaceHolder or ""
    local pointer = info.pointer or info.Pointer or info.flag or info.Flag or nil
    local callback = info.callback or info.callBack or info.Callback or info.CallBack or function()end
    ...
    local textbox = {axis = section.currentAxis, current = def}
    ...
    local textbox_title = utility:Create("TextLabel", {Vector2.new(4,textbox.axis - 15), section.section_frame}, {
        Text = name,
        Size = theme.textsize,
        Font = theme.font,
        Color = theme.textcolor,
        OutlineColor = theme.textborder,
        Position = utility:Position(0, 4, 0, textbox.axis - 15, section.section_frame),
        Visible = page.open
    }, section.visibleContent)

    library.colors[textbox_title] = {
        OutlineColor = "textborder",
        Color = "textcolor"
    }

    local textbox_value = utility:Create("TextLabel", {Vector2.new(4,0), textbox_frame}, {
        Text = def,
        Size = theme.textsize,
        Font = theme.font,
        Color = theme.textcolor,
        OutlineColor = theme.textborder,
        Center = true,
        Position = utility:Position(0, 4, 0, 0, textbox_frame),
        Visible = page.open
    }, section.visibleContent)
    ...
    library.began[#library.began + 1] = function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible then
            if utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + textbox.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + textbox.axis + 20}) and not window:IsOverContent() then
                task.spawn(function()
                    textbox_frame.Color = theme.darkcontrast
                    wait(0.15)
                    textbox_frame.Color = theme.lightcontrast
                end)

                local inputInstance = Instance.new("TextBox")
                inputInstance.Text = textbox.current
                inputInstance.ClearTextOnFocus = false
                inputInstance.Parent = game.CoreGui
                inputInstance:CaptureFocus()
                
                inputInstance.FocusLost:Connect(function(enterPressed)
    ...
    section.currentAxis = section.currentAxis + 35
    ...
    function sections:ButtonHolder(info)
        ...
        for i=1, 2 do
            ...
            --
            library.began[#library.began + 1] = function(Input)
                if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and button_outline.Visible and window.isVisible and utility:MouseOverDrawing({section.section_frame.Position.X + (i == 2 and (section.section_frame.Size.X/2) or 0), section.section_frame.Position.Y + button.axis, section.section_frame.Position.X + section.section_frame.Size.X - (i == 1 and (section.section_frame.Size.X/2) or 0), section.section_frame.Position.Y + button.axis + 20}) and not window:IsOverContent() then
                    task.spawn(function()
                        utility:LoadImage(button_gradient, "gradientdown", "https://i.imgur.com/XYVD9Mh.png")
                        --
                        task.wait(0.15)
    ...
    function sections:Dropdown(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and dropdown_outline.Visible then
                ...
                elseif utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + dropdown.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + dropdown.axis + (name and (15 + 20) or (20))}) and not window:IsOverContent() then
                    task.spawn(function()
                        utility:LoadImage(dropdown__gradient, "gradientdown", "https://i.imgur.com/XYVD9Mh.png")
                        --
                        task.wait(0.15)
                        --
                        utility:LoadImage(dropdown__gradient, "gradient", "https://i.imgur.com/XYVD9Mh.png")
            ...
            elseif (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and dropdown.open then
                dropdown.open = not dropdown.open
                utility:LoadImage(dropdown_image, "arrow_down", "https://i.imgur.com/tVqy0nL.png")
                --
                for i,v in pairs(dropdown.holder.drawings) do
        ...
        if max then
            library.ended[#library.ended + 1] = function(Input)
                if dropdown.scrolling and dropdown.scrolling[1] and (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
    ...
    function sections:Multibox(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and window.isVisible and multibox_outline.Visible then
                ...
                elseif utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + multibox.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + multibox.axis + (name and 15 or 0) + 20}) and not window:IsOverContent() then
                    task.spawn(function()
                        utility:LoadImage(multibox__gradient, "gradientdown", "https://i.imgur.com/XYVD9Mh.png")
                        --
                        task.wait(0.15)
                        --
                        utility:LoadImage(multibox__gradient, "gradient", "https://i.imgur.com/XYVD9Mh.png")
            ...
            elseif (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and multibox.open then
                multibox.open = not multibox.open
                utility:LoadImage(multibox_image, "arrow_down", "https://i.imgur.com/tVqy0nL.png")
                --
                for i,v in pairs(multibox.holder.drawings) do
    ...
    function sections:List(info)
        ...
        --
        library.ended[#library.ended + 1] = function(Input)
            if list.scrolling[1] and (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
